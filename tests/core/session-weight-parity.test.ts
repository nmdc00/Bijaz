...
describe('session weight parity between autonomous and mentat', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it.each([
  ...
  ])('matches session + weight at %s', async (at) => {
    const limiter = {
    ...
    const mentatContext = deriveSessionContext({ at, markets: [] });

    expect(reasoning).toContain(`session=${mentatContext.session}`);
    expect(reasoning).toContain(`sessionWeight=${mentatContext.sessionWeight.toFixed(2)}`);
  });
});
