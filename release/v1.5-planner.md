...
- Avoid hard dependency on a single web search provider in production paths.
...
11. `feat/v1.5-web-search-provider-resilience`
12. `feat/v1.5-integration-acceptance-tests`
...
## 11) Web Search Provider Resilience
Branch: `feat/v1.5-web-search-provider-resilience`

Deliverables:
- Add `web_search` provider abstraction with configurable fallback order.
- Add provider adapters and failure classification for quota/rate-limit/timeouts.
- Add shared cache layer (TTL + deterministic key normalization) for repeated queries.
- Add budget controls (per-turn/per-day caps) and circuit breaker per provider.
- Add provider telemetry and operator visibility for usage, failover, and error rates.
- Capture final response provenance with provider/source metadata.
- Follow implementation sketch in `release/v1.5-web-search-resilience-implementation.md`.

Acceptance criterion:
- If primary search provider is exhausted, search continues through configured fallback provider without hard failure.

Test requirement:
- Tests for fallback ordering, cache hits, quota/rate-limit handling, budget ceilings, and provenance fields.

## 12) Integration and Acceptance Harness
...
- Web search fallback/caching/budget controls prevent single-provider quota outages from disabling intel workflows.
