...
- Time-sensitive answers must run proactive refresh before final regime conclusions.
...
11. `feat/v1.4-proactive-refresh-gate`
...
## 11) Proactive Refresh Gate for Time-Sensitive Queries
Branch: `feat/v1.4-proactive-refresh-gate`

Deliverables:
- Add intent detection for time-sensitive prompts (`latest`, `today`, `this week`, `bull/bear`, macro regime questions).
- Add pre-answer refresh pipeline that runs market + funding/OI + headline fetches in parallel.
- Add freshness TTL cache to reduce repeat-latency on near-term follow-ups.
- Add strict fail-closed option when required refresh cannot be completed.
- Add response metadata requirements: `as_of` timestamp and source summary for refreshed answers.

Acceptance criterion:
- Time-sensitive prompts cannot return stale regime/macro answers without either fresh evidence or explicit fail-closed message.

Test requirement:
- Tests for intent detection, TTL freshness behavior, fail-closed path, and `as_of`/source response contract.
...
10. `feat/v1.4-proactive-refresh-gate`
11. `feat/v1.4-integration-acceptance-tests`
...
- Time-sensitive prompts pass proactive freshness gates with timestamped/source-attributed responses.
